...
static bool packet_hsync_positive = HSTX_PACKET_DEFAULT_SYNC_POSITIVE;
static bool packet_vsync_positive = HSTX_PACKET_DEFAULT_SYNC_POSITIVE;
#endif
                                          ...
                                          bool aspect_16_9)
{
    ...
    packet->subpacket[0][1] = 0x10;                       // A=1: active format information is valid.
    packet->subpacket[0][2] = aspect_16_9 ? 0x28 : 0x18; // M=16:9 or 4:3; R=8 (same as coded frame).
    packet->subpacket[0][3] = 0x00;
    packet->subpacket[0][4] = vic;
    packet->subpacket[0][5] = pixel_repetition & 0x0F;
...
#if PICO_HDMI_RT_RUNTIME_MODE_ATTRS
    const bool hsync_positive = packet_hsync_positive;
    const bool vsync_positive = packet_vsync_positive;
#else
    const bool hsync_positive = HSTX_PACKET_DEFAULT_SYNC_POSITIVE;
    const bool vsync_positive = HSTX_PACKET_DEFAULT_SYNC_POSITIVE;
#endif
