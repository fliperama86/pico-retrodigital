...
// Pixel clock 74.4 MHz from the project's 372 MHz clock; vreg = 1.30V.
...
static void build_all_command_lists(const video_mode_t *mode)
{
    ...
    // AVI InfoFrame: keep VIC 1/4 for the standard active formats. The two PC
    // descriptors use VIC 0 with explicit 4:3 active-format metadata.
    hstx_packet_t packet;
    hstx_data_island_t island;
    const bool mode_is_240p = mode->v_active_lines == 240 && mode->h_active_pixels == 1280;
    const bool mode_is_cea_720p = mode->v_active_lines == 720 && mode->h_active_pixels == 1280;
    uint8_t vic = (mode->v_active_lines == 480 && mode->h_active_pixels == 640) ? 1 : mode_is_cea_720p ? 4 : 0;
    ...
    if (mode_is_240p) {
        ...
        hstx_packet_set_avi_infoframe_aspect(&packet, vic, pixel_repetition, mode_is_cea_720p);
    }
    hstx_encode_data_island(&island, &packet, false, di_hsync_active);
